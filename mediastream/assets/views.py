...
from mediastream.assets.forms import UploadFileForm
...
def upload_file(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if inspobj.mimetype == 'application/zip':
                ...
                for member in myzip.namelist():
                    ...
                    count += 1
                messages.info(request, 'Found {0} file{1} in zip file {2}.'.format(count, '' if count == 1 else 's', request.FILES['file'].name))
            elif inspobj.mimetype.startswith('audio/'):
                stash.seek(0)
                files.append((stash, inspobj))
                messages.info(request, 'Found {0} to be a Track of type {1}.'.format(request.FILES['file'].name, inspobj.mimetype))
            else:
                messages.error(request, 'Could not figure out what to do with {0} of type {1}.'.format(request.FILES['file'].name, inspobj.mimetype))
        ...
        form = UploadFileForm()
    return render_to_response('assets/upload.html', {'form': form}, context_instance=RequestContext(request))
